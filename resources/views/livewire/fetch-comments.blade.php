    ...
    <div class="p-2 flex justify-between  rounded bg-slate-100 shadow-md my-4">
        ...
        <div class="flex my-auto mx-4 space-x-5">
            <div>
                ...
                </svg> <b> {{ $comment->likes->count() }}</b>

            </div>
            <div wire:click="openModal({{$comment->id}})" class="flex flex-col"><i class='far fa-comment-dots cursor-pointer ' style='font-size:24px'></i>
                @if( $comment->answers)
                <b> {{ $comment->answers->count() }}</b>
                @endif
            </div>
        </div>
    </div>
